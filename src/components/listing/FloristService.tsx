...
import svgPaths from "../../imports/svg-2m13agmwn2";
import PhotoEditOverlay from "./PhotoEditOverlay";

interface FloristServiceProps {
...
export default function FloristService({ onNavigate, onBack, onNext, initialData }: FloristServiceProps) {
  const [formData, setFormData] = useState({
    serviceName: initialData?.serviceName || "Petite soiree",
    ...
    included: initialData?.included || {
      bridalBouquet: { selected: true, quantity: 1 },
      bridesmaidsbouquet: { selected: true, quantity: 1 },
      corsages: { selected: true, quantity: 1 },
      flowerCrown: { selected: false, quantity: 0 },
      boutonnieres: { selected: true, quantity: 1 },
      other: { selected: true, name: "", quantity: 1 }
    },
    flowerTypes: initialData?.flowerTypes || ""
  });
  ...
  const handleIncludedToggle = (item: string) => {
    setFormData(prev => ({
      ...
      included: {
        ...
        [item]: {
          ...prev.included[item],
          selected: !prev.included[item].selected
        }
      }
    }));
  };
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                {Object.entries(formData.included).map(([key, item]) => (
                    ...
                    {item.selected && (
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="w-8 h-8 rounded-full border border-[#8c97ac] flex items-center justify-center text-[#47567b] hover:bg-gray-50"
                        >
                          -
                        </button>
                        <span className="w-12 text-center text-[#47567b]">{item.quantity}</span>
                        <button
                          onClick={() => handleQuantityChange(key, item.quantity + 1)}
                          className="w-8 h-8 rounded-full border border-[#8c97ac] flex items-center justify-center text-[#47567b] hover:bg-gray-50"
                        >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, flowerTypes: e.target.value }))}
                className="w-full px-4 py-3 border border-[#8c97ac] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#47567b] focus:border-transparent text-[#47567b]"
