...
interface FloristAuthProps {
  onNavigate: (view: string) => void;
  onSuccess: () => void;
}

export default function FloristAuth({ onNavigate, onSuccess }: FloristAuthProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        // For sign up, we need to create a vendor account with florist type
        await signUp(email, password, 'vendor', 'florist');
        // Set vendor type in localStorage for the app to recognize
        localStorage.setItem('zinnia_vendorType', 'florist');
      } else {
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-[#47567b] text-sm" style={{ fontFamily: 'Montserrat, sans-serif' }}>
            Sign in to manage your florist listing
          </p>
