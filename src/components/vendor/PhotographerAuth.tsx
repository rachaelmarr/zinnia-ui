...
export default function PhotographerAuth({ onNavigate, onSuccess }: PhotographerAuthProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        // For sign up, we need to create a vendor account with photographer type
        await signUp(email, password, 'vendor', 'photographer');
        // Set vendor type in localStorage for the app to recognize
        localStorage.setItem('zinnia_vendorType', 'photographer');
      } else {
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-[#47567b] text-sm" style={{ fontFamily: 'Montserrat, sans-serif' }}>
            Sign in to manage your photography listing
          </p>
          ...
          <div className="mt-4">
            <button
              type="button"
              onClick={() => {
                setEmail('photographer@test.com');
